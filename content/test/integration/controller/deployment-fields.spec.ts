...
describe("Integration - Deployment Fields", () => {
    ...
    it('When deployments fields filter is used, then the result is the expected', async() => {
        ...
        const withAll = await fetchDeployment(DeploymentField.AUDIT_INFO, DeploymentField.CONTENT, DeploymentField.POINTERS, DeploymentField.METADATA)
    ...
    function assert(deployment: Partial<Deployment>, check: { auditInfo?: boolean, metadata?: boolean, content?: boolean, pointers?: boolean }) {
        ...
        if (opts.auditInfo) expect(deployment.auditInfo).toBeDefined(); else expect(deployment.auditInfo).toBeUndefined();
        if (opts.content) expect(deployment.content).toBeDefined(); else expect(deployment.content).toBeUndefined();
        if (opts.metadata) expect(deployment.metadata).toBeDefined(); else expect(deployment.metadata).toBeUndefined();
        if (opts.pointers) expect(deployment.pointers).toBeDefined(); else expect(deployment.pointers).toBeUndefined();
    }
