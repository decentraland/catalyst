...
loadTestEnvironment()('Errors during sync', (testEnv) => {
  ...
  describe('Deploy an entity on server 1', function () {
    beforeEach(async function () {
      ...
      this.accessCheckerStub2 = stub(this.server2.components.accessChecker, "hasAccess");
      this.accessCheckerStub1.returns(Promise.resolve([]));
      this.accessCheckerStub2.onCall(0).returns(Promise.resolve(['anyError']));
      this.accessCheckerStub2.onCall(1).returns(Promise.resolve([]));
    ...
    afterEach(async function () {
      ...
      this.accessCheckerStub2.restore()
    })

    it('stores it as failed deployment locally', async function () {
      await awaitUntil(() => assertDeploymentFailed(this.server2, FailureReason.DEPLOYMENT_ERROR, this.controllerEntity))
    });
    ...
    describe('fix the failed entity on server2 after a new one was correctly deployed', function () {
      ...
      beforeEach(async function () {
        ...
        this.anotherEntityCombo = await buildDeployDataAfterEntity(
