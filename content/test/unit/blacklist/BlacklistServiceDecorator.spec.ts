...
describe("BlacklistServiceDecorator", () => {
    ...
    it(`When an entity is blacklisted, then it is marked as so on the audit info`, async () => {
        ...
        expect(auditInfo.ethAddress).toEqual(MockedMetaverseContentService.AUDIT_INFO.ethAddress)
        expect(auditInfo.signature).toEqual(MockedMetaverseContentService.AUDIT_INFO.signature)
