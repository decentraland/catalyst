...
export class DenylistServiceDecorator implements MetaverseContentService {
  ...
  async deployEntity(
    ...
    const hashedFiles = await this.repository.reuseTaskIfPresent(
      task,
      (db) => {
        return this.validateDeployment(db.denylist, files, entityId, auditInfo)
      },
  ...
  async getDeployments(task?: Database, options?: DeploymentOptions): Promise<PartialDeploymentHistory<Deployment>> {
    return this.repository.reuseTaskIfPresent(
      task,
      async (db) => {
        const deploymentHistory = await this.service.getDeployments(db, options)
        ...
        const queryResult = await this.denylist.areTargetsDenylisted(db.denylist, allTargets)
