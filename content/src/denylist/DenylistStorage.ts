...
export class DenylistStorage {
    ...
    async getAllDenylists(): Promise<Map<DenylistTarget, DenylistMetadata>> {
        ...
        const denylists: Promise<[string, DenylistMetadata | undefined]>[] = allTargetsAsId.map<Promise<[string, DenylistMetadata | undefined]>>(async targetAsId => [targetAsId, await this.retrieveMetadata(targetAsId)])
