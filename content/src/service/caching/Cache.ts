...
export class Cache<Key, Value> {
    ...
    constructor(size: number) {
        this.internalCache = new LRU({ max: size, maxAge: ms('3d') })
    }
    ...
    async get(keys: Key[], orCalculate: (keys: Key[]) => Promise<Map<Key, Value | undefined>>): Promise<Value[]> {
        ...
        const calculated = await orCalculate(missing)
...
export class CacheByType<Key, Value> {
    ...
    get(type: EntityType, keys: Key[], orCalculate: (type: EntityType, keys: Key[]) => Promise<Map<Key, Value | undefined>>): Promise<Value[]> {
