...
export class IntPropertyMapper implements SystemPropertyMapper<number> {
    toString(value: number): string { return `${value}` }
    fromString(value: string): number { return parseInt(value) }
}
...
export class JSONPropertyMapper<T> implements SystemPropertyMapper<T> {
    toString(value: T): string { return JSON.stringify(value) }
    fromString(value: string): T { return JSON.parse(value) }
}