    ...
    async () => {
      ...
      for (const server of servers) {
        try {
          ...
          LOGGER.debug(`Tried to ${description} on '${server.getContentUrl()}' but it failed because of: ${error}`)
        }
      }
...
function reorderAccordingToPreference(
  ...
  if (preferred) {
    const newOrder = activeServers.filter((server) => server.getContentUrl() != preferred.getContentUrl())
