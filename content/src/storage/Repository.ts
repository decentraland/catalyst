...
export class Repository {
  ...
  private runInternal<T>(execution: (db: Database) => Promise<T>, options?: ExecutionOptions): Promise<T> {
    const { priority } = { ...DEFAULTS, ...options }
    return this.queue.addDatabaseRequest(priority, () => execution(this.db))
  }
}

const DEFAULTS: CompleteExecutionOptions = {
  priority: DB_REQUEST_PRIORITY.LOW
}

type ExecutionOptions = Partial<CompleteExecutionOptions>
