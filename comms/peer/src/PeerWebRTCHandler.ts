...
export class PeerWebRTCHandler extends EventEmitter<PeerWebRTCEvent> {
  ...
  constructor(providedConfig: PeerWebRTCConfig) {
    ...
    this.config = {
      ...
      oldConnectionsTimeout: providedConfig.oldConnectionsTimeout ?? (providedConfig.peerConnectTimeout ?? PEER_CONSTANTS.DEFAULT_PEER_CONNECT_TIMEOUT) * 10,
      handshakeValidator:
  ...
  private handleSignal(peerData: ConnectedPeerData) {
    ...
    return (data: SignalData) => {
      ...
      if (this.isReadyToEmitSignals()) {
        ...
        this.log(LogLevel.WARN, "Ignoring connection signal since the peer is not ready to emit signals yet", peerData, data);
      }
    };
  }
  ...
  awaitConnectionEstablished(timeoutMs: number = 10000): Promise<void> {
    ...
    setTimeout(() => {
      result.isPending && result.reject(new Error(`[${this.maybePeerId()}] Awaiting connection to server timed out after ${timeoutMs}ms`));
    }, timeoutMs);
