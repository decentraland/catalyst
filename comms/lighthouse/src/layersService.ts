...
export class LayersService {
  ...
  getLayerTopology(layerId: string) {
    return this.layers[layerId].peers.map((it) => ({
      ...this.peersService.getPeerInfo(it),
      connectedPeerIds: this.peersService.getConnectedPeers(it)
    }))
  }
  ...
  getOptimalConnectionsFor(peerId: string, targetConnections: number, maxDistance: number) {
    ...
    if (peerInfo.layer && peerInfo.position) {
      return {
        ...
        optimalConnections: this.peersService.getOptimalConnectionsFor(
          peerInfo,
          this.getLayerPeers(peerInfo.layer),
          targetConnections,
          maxDistance
        )
      }
