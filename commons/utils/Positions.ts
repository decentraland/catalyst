...
export function discretizedPositionDistance<PositionType>(a: PositionType, b: PositionType) {
  let dx = 0;
  let dy = 0;
  let dz = 0;
  ...
  return Math.floor((dx * dx + dy * dy + dz * dz) / (DISCRETIZE_POSITION_INTERVAL * DISCRETIZE_POSITION_INTERVAL));
}