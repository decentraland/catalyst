...
- `MAX_IN_FLIGHT_UPLOAD_BYTES`: Max POST /entities body bytes buffered in memory at once across all clients; excess uploads get a `503`. Partial batches are exempt from the per-IP daily quota (not the per-minute burst limit) and bounded by this and their account's byte quotas instead. Must fit one maximum-size request's peak: `MAX_UPLOAD_TOTAL_SIZE` plus a copy of one file (up to `MAX_UPLOAD_FILE_SIZE`). Default: "4294967296" (4 GiB)
