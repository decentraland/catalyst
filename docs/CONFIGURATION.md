...
- `MAX_IN_FLIGHT_UPLOAD_BYTES`: Max POST /entities body bytes spooled to temporary files (under `<STORAGE_ROOT_FOLDER>/contents/_uploads`) at once across all clients; excess uploads get a `503`. Partial batches are exempt from the per-IP request limits and bounded by this instead. Must be at least `MAX_UPLOAD_TOTAL_SIZE`. Default: "4294967296" (4 GiB)
