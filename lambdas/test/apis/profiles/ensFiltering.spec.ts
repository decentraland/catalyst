...
describe('Ensure ENS filtering work as expected', () => {
  it(`Ensure Address case is ignored when retrieving ENS`, async () => {
    ...
    const namesOriginal = await filterENS(fetcher, DEFAULT_ENS_OWNER_PROVIDER_URL_ROPSTEN, originalAddress, [
      'marcosnc',
      'invalid_name'
    ])
    expect(namesOriginal.length).toEqual(1)

    const namesUpper = await filterENS(fetcher, DEFAULT_ENS_OWNER_PROVIDER_URL_ROPSTEN, originalAddress.toUpperCase(), [
      'marcosnc',
      'invalid_name'
    ])
    expect(namesUpper).toEqual(namesOriginal)

    const namesLower = await filterENS(fetcher, DEFAULT_ENS_OWNER_PROVIDER_URL_ROPSTEN, originalAddress.toLowerCase(), [
      'marcosnc',
      'invalid_name'
    ])
