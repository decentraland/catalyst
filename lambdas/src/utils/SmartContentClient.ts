...
export class SmartContentClient implements ContentAPI {
    ...
    async fetchFullHistory(query?: { from?: Timestamp, to?: Timestamp, serverName?: ServerName }, options?: RequestOptions): Promise<LegacyDeploymentHistory> {
        const client = await this.getClient();
        return client.fetchFullHistory(query, options);
    }

    async fetchHistory(query?: {from?: Timestamp, to?: Timestamp, serverName?: ServerName, offset?: number, limit?: number }, options?: RequestOptions): Promise<LegacyPartialDeploymentHistory> {
        const client = await this.getClient();
    ...
    private async getClient(): Promise<ContentAPI> {
        if (!this.contentClient) {
            ...
            try {
                ...
                await fetcher.fetchJson(`${SmartContentClient.INTERNAL_CONTENT_SERVER_URL}/status`, { attempts: 6, waitTime: '10s' } )
